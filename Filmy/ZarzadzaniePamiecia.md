...
Pamięć operacyjna - czyli ogólnie rzecz biorąc cała pamięć adresowana i dostępna przez procesor. Pamieć operacyjna to nie tylko znana wszystkim pamięć RAM, ale też inne rodzaje pamięci - np: ROM czy EPROM). My w dzisiejszym odcinku skupimy się właśnie na pamięci RAM (ang. Random Access Memory) jako, że przytłaczająca większość aplikacji (gry, programy do edycji, przeglądarki internetowe etc.) w czasie swojej pracy wykorzystują właśnie ten rodzaj pamięci. Innymi słowy - RAM to miejsce na najróżniejsze dane dla programów, które aktualnie są uruchomione na komputerze. Kiedy, przykładowo, uruchamiamy grę, to jej "ładowanie" odnosi się właśnie do tego, że bierze ona dane, takie jak np: tekstury, modele, dźwięki z wolnej pamięci dodatkowej, najcześciej **z dysku** i zapisuje je do znacznie szybszej pamięci operacyjnej. Każdemu programowi, a dokładniej procesowi, system operacyjny przydziela pulę pamięci, czyli tzw. pamięć wirtualną procesu. Kiedy program potrzebuje więcej miejsca, to wysyła odpowiednią prośbę do systemu operacyjnego, a kiedy dane miejsce nie jest już używane, to niejako mówi systemowi: "*tego miejsca już nie używam, rób z tym co chcesz*".


W sytuacji, kiedy na komputerze używamy większej liczby programów na raz, tej pamięci może zabraknąć. Niektórych barier się nie przeskoczy, bo ciężko jest oczekiwać, że na komputerze z zainstalowanymi... 2GB RAMu uruchomimy jednocześnie przeglądarkę, grę, film w tle i program do edycji filmów. To znaczy... zaraz, zaraz! Przeskoczymy! Twórcy systemów operacyjnych (np: Windowsa) postarali się o to, żeby  w powyższej sytuacji udało się uruchomić tyle alikacji a komputer nie powiedział "Nie na da! Kaput!". To nie magia tylko inżynieria. W systemie Windows istnieje coś takiego jak plik stronicowania. Jest to całkiem sporych rozmiarów plik, który jest w stanie działać jak dodatkowa pula pamięci dla RAMu. Niestety, taki plik znajduje się na pamięci dyskowej (uzytkownicy Windowsa mogą znaleźć go na dysku C). Z racji położenia nie w bardzo szybkiej pamięci RAM a na wolnym (w stosunku do RAM) dysku - dostęp do danych będzie sporo wolniejszy. Co gorsza, system w kółko maltretujący dysk twardy (czy to SSD czy HDD) spowolni procesy odczytywania i zapisywania danych wszystkiego co jest już uruchomnione! Cóż, coś za coś. Teraz już wiecie dlaczego czasem komputer "zamula". Ale my nie o tym dzisiaj.

Mimo, że dzisiaj komputery takie szybkie i pojemne - zawsze trzeba pilnować zarządzania pamięcią, bo ta (jak już wyżej wspomnieliśmy może się skończyć co poskutkuje dużym spowolnieniem pracy komputera. Przyjrzyjmy się pierwszemu problemowi z pamięcią, a raczej błędnie zaprojektowanej aplikacji. Potocznie chodzi o problem puchnięcia pamięci. Weźmy na przykład taką prostą sytuację: gra strategiczna, w której możemy tworzyć nowe jednostki wojowników. W trakcie gry wiele jednostek jest tworzonych, czyli dla każdej nowej jednostki jest rezerwowany kolejny mały wycinek w pamięci RAM - wiadomo trzeba gdzieś przechować aktualną pozycję jednostki, ilość punktów życia czy uzbrojenie. Jak to na wojnie - część z nich umrze podczas walki. Wtedy pamięć odpowiedzialna za martwych wojów powinna zostać zwolniona, bo po co w uproszczeniu oczywiście gra ma się nimi przejmować? Kolejna dygresja - dlaczego w latach głównie 1990-2000 w grach FPS korpusy poległych wrogów, ślady po kulach na ścianach czy rozbite elementy gry potrafiły zniknąć po czasie? Ano dlatego, że w tamtych czasam pamięć RAM dostępna dla gier była sporo mniejsza. Dlatego trzeba było zrezygnować z długiego trzymania informacji o niepotrzebnych obiektach. Inaczej szybko skończyła by się pamięć RAM. Kolejna zagadka z dzieciństwa rozwiązana ;). Z drugiej strony nawet dzisiejsze FPSy czy strategie często "po cichutku" zamiatają pod dywan takie rzeczy jak łuski po nabojach, elementy rozwalonych mebli czy zwłoki poległych. Ale wróćmy do meritum: Jeśli programista nie umie poprawnie zarządzać pamięcią i nie będzie zwalniał jej po każdym takim poległym w boju wojowniku, to pamięć użyta przez grę będzie z czasem coraz bardziej rosnąć - mimo, że o bitwie już dawno zapomniano. Stąd potoczne określenie o puchnięciu pamięci.W końcu proces z grą będzie zajmował na tyle dużo pamięci, że do akcji wkroczy plik stronicowania i gra stanie się niegrywalna. Najgorszy przykład? Użytkownik dalej **gra w gre**, plik stronicowania też puchnie - aż do momentu gdy na dysku C zabraknie miejsca. Wtedy stanie się jedna z rzeczy: Gra po kolejnej prośbie o pamięć dostanie od systemu tylko chamskie *"Nie dam!"* i sama zakończy działanie, albo system zakończy proces bo gra nie będzie wiedziałą co zrobić. Z drugiej strony system też może zgłupieć, bo potrzebuje coś zapisać do pamięci, której już nie ma...Ach, muszę kiedyś spróbować doprowadzić Windowsa na skraj załamania nerwowego. W sytuacji puchnięcia pamięci z powodu braku zwalniania niepotrzebnych zasobów nie jest źle. W przypadku zamknięcia aplikacji cała zajęta przez aplikację pamięć zostanie zwolniona, znaczy się wróci do puli dostępnej pamięci dla systemu i innych aplikacji. Gorzej jeśli nastąpi:

WYCIEK PAMIĘCI.

Z sytuacjami, gdzie może nastąpić **wyciek pamięci** (bo tak właśnie się to nazywa) programiści spotykają się... stale i przez to jest to takie ważne. Bardziej szczegółowo, wyciek pamięci to sytuacja, w której niepotrzebna, nigdzie już nieużywana pamięć pozostaje zabrana przez proces a my tracimy ostatnią szansę, żeby ją zwolnić, bo nie mamy już żadnego "odnośnika" (czy, mówiąc bardziej po programistycznemu, wskaźnika) do tego, gdzie ta pamięć się znajduje, więc będzie ona sobie tak trwała, zaalokowana, nieużywana do końca życia procesu.
